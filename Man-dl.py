...
def start():
    ...
    search_term = input('Enter the manga name or enter "U" to check for updates on previously updates series: \033[92m ')
    if search_term == "U":
        Update()
    else:
...
def generate_cbz(folder, image_names, chapter_name, manga_name):
    """Compress the downloaded images into a .zip file"""
    file = folder.joinpath(manga_name + " " + chapter_name + ".zip")
    if image_names:
        z = zipfile.ZipFile(file, 'w')
        for image in image_names:
            z.write(image)
    else:
        print("Error! No images found, ZIP not made")
